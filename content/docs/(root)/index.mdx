...
Install a **block** — a ready-made Open Graph image — with the shadcn CLI, drop
it into an `ImageResponse`, and you have a polished social card in minutes —
fully owned, fully editable code. Blocks are composed from a small set of
Satori-safe **components** (avatar, badge, brand mark, grid lines) that you can
also use to build your own.
...
- 🖼️ **22 OG image blocks** - Blog, changelog, event, product, quote, shadcn registry cards, and more
- 🧩 **Satori-safe components** - Avatar, badge, brand mark, and grid lines primitives
...
1. Each block lives at `registry/og/<name>.tsx`, and each component at `registry/og/ui/<name>.tsx`
2. Both are listed in `registry.json` — blocks as `registry:block` items that depend on the components they use
3. `pnpm registry:build` emits installable JSON to `public/r`

Users then install any block, and the CLI pulls in its components:
...
Absolutely. Add a file to `registry/og/`, list it in `registry.json`, write an
MDX doc, and run `pnpm registry:build`. Import components from `./ui/<name>` so
the file keeps working after installation.
