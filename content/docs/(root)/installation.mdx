---
title: Installation
description: Install an ogimagecn block and render it as an Open Graph image.
---

## Install a block

Pick a block from the [gallery](/docs/blocks) and add it with the shadcn CLI.
The block is copied into your project at `components/og/<name>.tsx`, and the
[components](/docs/components) it uses land in `components/og/ui/`.
...
Create `registry/og/yours.tsx`. Use **flexbox + inline styles only** so it
renders in both Satori and the browser. Import components with relative paths
such as `./ui/badge` — blocks install to `components/og/` and components to
`components/og/ui/`, so the import keeps resolving after installation.
  ...
  "items": [
    {
      ...
      "type": "registry:block",
      "title": "Yours",
      "description": "Your custom OG image.",
      "registryDependencies": ["https://ogimagecn.com/r/badge.json"],
      "files": [
        {
          "path": "registry/og/yours.tsx",
